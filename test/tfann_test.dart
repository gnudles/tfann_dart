...
final List<Float32x4List> Lweight_tfann_evaluate_0 = [
  Uint32List.fromList([1061734424, 1068319737, 3197279448, 0, 0, 0])
      .buffer
      .asFloat32x4List(),
  Uint32List.fromList([3212343254, 3214242561, 3216839713, 0, 0, 0])
      .buffer
      .asFloat32x4List(),
  Uint32List.fromList([3218877343, 3218807543, 3218903785, 0, 0, 0])
      .buffer
      .asFloat32x4List(),
  Uint32List.fromList([3223833271, 3223437569, 3224038821, 0, 0, 0])
      .buffer
      .asFloat32x4List(),
  Uint32List.fromList([1066640933, 3211839774, 1073016495, 0, 0, 0])
      .buffer
      .asFloat32x4List()
];
final Float32x4List Lbias_tfann_evaluate_0 = Uint32List.fromList(
        [1018821290, 1076714006, 3221889414, 1059659408, 1063215464, 0, 0, 0])
    .buffer
    .asFloat32x4List();
final List<Float32x4List> Lweight_tfann_evaluate_1 = [
  Uint32List.fromList(
          [1067653208, 3219956846, 3223325029, 1078770049, 1061053443, 0, 0, 0])
      .buffer
      .asFloat32x4List(),
  Uint32List.fromList(
          [1067927215, 3218923674, 1050408590, 1065383446, 1060567289, 0, 0, 0])
      .buffer
      .asFloat32x4List(),
  Uint32List.fromList(
          [3182724903, 1038259357, 3222743775, 1058908675, 3172068864, 0, 0, 0])
      .buffer
      .asFloat32x4List(),
  Uint32List.fromList(
          [3216437517, 1072719072, 1039901040, 3223588606, 3210176461, 0, 0, 0])
      .buffer
      .asFloat32x4List()
];
final Float32x4List Lbias_tfann_evaluate_1 = Uint32List.fromList(
        [3217132268, 3206537276, 3205234870, 3215634523, 0, 0, 0])
    .buffer
    .asFloat32x4List();
...
List<double> tfann_evaluate(List<double> inData) {
  ...
  for (int i = 0; i < 2; ++i) currentTensor[i] += Lbias_tfann_evaluate_0[i];
  currentTensor[0] = slqX4(currentTensor[0]);
  outputTensor[4] = slq(outputTensor[4]);
...
void main() {
  test('sine net', () async {
    Random r = Random();
    final sine_net =
        TfannNetwork.full([1, 24, 36, 1], activation: ActivationFunctionType.lelq);
    ...
    for (int i = 0; i < 400; ++i) {
      var x = r.nextDouble() * 6.29 - 3.145;
      print("$x : ${sine_net.calculateMeanAbsoluteError(
